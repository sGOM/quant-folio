...
class RiskLayer(BaseModel):
    ...
    selection·weighting 으로 목표비중을 산정한 뒤, 체결 직전에 순서대로 적용된다:
      ...
      1-b) 섹터 집중 한도(max_sector_pct): 동일 업종 합산 목표비중 상한. 초과 섹터의
         종목들을 섹터 내 비례로 축소하고, 잘라낸 초과분을 여력 있는 섹터의 종목들에
         재분배한다(여력 없으면 현금). 업종 동반 급락(체계적 섹터) 리스크를 제한한다.
    ...
    섹터 한도(max_sector_pct)는 KRX MDC 업종분류현황(MDCSTAT03901)을 종목→업종 매핑
    소스로 구현했다(시장당 요청 1회로 전종목 업종 확보). 인증 세션 미확보로 맵이 비면
    섹터 캡은 조용히 미적용으로 폴백한다.
    """
    ...
    max_sector_pct: float | None = Field(
        default=None, gt=0, le=1,
        description="동일 업종 합산 목표비중 상한(0~1, 예: 0.30=30%). None=미적용. 초과 섹터 "
        "종목을 섹터 내 비례로 축소하고 초과분을 여력 섹터에 재분배, 여력 없으면 현금. "
        "종목→업종 매핑은 KRX MDC 업종분류현황(맵 미확보 시 미적용).",
    )
