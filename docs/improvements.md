...
| 3 | 체결 모델 정밀화 id=23/24 재검증 | §4 코드는 완료 — `price_limit_model=True` 로 영향도 판정(성과 변화 미미하면 근사 유지로 종결) |
| 4 | 팩터 섹터 중립화 id=23 A/B (§20) | §20 코드는 완료 — `neutralize="sector"`로 PIT KOSPI200 A/B(판정은 alpha/Sharpe)해 업종 쏠림 분리 효과 확정 |
...
| 7 | alerts "더보기" 프론트 UI (§21 부수) | 백엔드·`listAlerts` offset 지원은 완료, `AlertCenter.tsx`에 이전 이력 로드 버튼 연동은 미착수 |

완료 이동: TTM A/B 재검증(§3, 2026-07-19) — 혼재로 옵트인 유지 종결.
