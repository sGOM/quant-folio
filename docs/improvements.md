...
**미착수 — 게이지 본체**: 신용융자잔고/시총, 레버리지 ETF 순자산/기초시총, 지수 집중도,
종목간 평균상관을 결합해 `base_exposure`를 **연속 스케일**(1.0→0.6)하는 부분. 이진 on/off가
아닌 이유는 기존 레짐 오버레이의 이진 스위치가 2026-07에 최악 특성(늦게 끄고 7-31 반등을
놓침)을 보였기 때문이다.

**착수 전제 — 1순위 지표 미확보**: 신용융자 잔고는 FreeSIS 증시자금 통계표에 **없고**(별도
OBJ_NM 미발견), 레버리지 ETF 순자산은 KRX MDC라 **KRX 로그인 세션이 필요**하다(무인증
HTTP 400). 이 둘 없이 착수하면 반쪽이다.
...
## 46. 취약성 게이지 오탐률 사전 측정 (§45 승격 조건)

사후 확증편향 방지가 목적. 2026년 7월 결과를 알고 지표를 고르면 무조건 맞는 지표가 나온다.
**임계값을 먼저 코드에 확정한 뒤** 2008·2011·2020·2022 등 다른 국면 전체에 돌려 오탐률을
센다. 게이지가 켜졌는데 폭락이 안 온 구간의 비율이 실용성을 결정한다.

오탐률 미측정 채택은 금지한다 — vol-harvest 게이트(id=32 전면 기각)·섹터 중립화(§20 기각)
전례가 있다. §45의 FreeSIS 2008년 소급으로 측정에 필요한 이력은 확보됐다.

## 47. id=23의 2026-07 구간 4-arm 검증 (P2 패닉 오버레이 자연실험)

VKOSPI 96.94(2009년 집계 이후 최고)·이틀 연속 서킷브레이커(사상 최초)·7-13 −8.95% 구간은
`metrics/panic.py`의 S1·S2·S5·S8이 거의 확실히 panic 라벨을 찍는 국면이라, **P2 패닉
