...
## 전체 점검 (2026-08-31, §64~§67)

Docker 기동 후 백엔드 pytest·프론트 lint/vitest/build 를 모두 실행한 상태에서
2026-08-19 재스캔 이후의 신규 표면(KIS 종목마스터 캐시 등)을 훑었다. 스위트는
전부 통과하고 있었고, 아래 4건이 나왔다.

## 64. `StockMetric` 의 0 sentinel — §50·§62 와 같은 계약이 인접 경로에 잔존 ✅

§50 이 `RecommendMember.price` 를, §62 가 `RecommendMember.market_cap` 을 0 sentinel
에서 `None` 으로 바꿨는데, 바로 옆 `/api/metrics/stocks` 경로는 범위 밖으로 남아 있었다
(§62 가 "인접한 것을 §67 로 미루면 이 부류가 재발한다"고 적어둔 그 패턴이다).

**해소(2026-08-31)**: `price`·`market_cap`·`avg_value_20` 셋 다 `Optional` 로 바꿨다.
`price` 는 유동성 필터를 통과한 종목이라도 종가만 결측일 수 있어 실제로 도달하던 경로고,
`market_cap`·`avg_value_20` 은 소스 부분 장애로 컬럼이 통째로 빠지면 해당 필터도 함께
비활성화돼 전 종목이 sentinel 로 채워지던 경로다. 프론트 `lib/api.ts` 는 이미 세 필드를
`number | null` 로 선언하고 `fmtKRW`/`fmtAmt` 로 렌더하며 `sortRows` 도 null 을 하단으로
보내므로 소비처 변경은 없었다. `routes/metrics.py` 의 `min_value`·`min_mcap` 추가 필터만
"결측은 더 엄격한 기준의 충족을 증명할 수 없다"는 규칙으로 탈락시키도록 가드했다.
회귀 테스트 2건 신설(`test_stocks_metrics.py`).

**부수**: 스크리너의 같은 자리(`screener.py` `market_cap`)는 조사해 보니 **도달 불가**
였다 — 시가총액 컬럼 부재는 위쪽에서 조기 반환이고, `cap_df["시가총액"] <= threshold`
가 NaN 행을 이미 떨어뜨린다. 계약을 바꾸는 대신(검증할 경로가 없다) 죽은 가드를 지우고
그 불변식을 테스트로 고정했다. 프론트에서는 `fmtSubscore` 가 `fmtNum(v, 2)` 의 중복이라
제거했다(§63 과 같은 계열).

## 65. KIS 종목마스터 스냅샷 거래일이 UTC 기준 ✅

`snapshot_stock_master` 가 `date.today()` 로 거래일을 찍고 있었다. 컨테이너 TZ 는 UTC 고
KRX 거래일은 KST 라, 야간 배치를 KST 00:00~09:00 으로 옮기면 스냅샷 거래일이 에러 없이
하루 어긋난다. `worker/tasks.py::_snapshot_target_date` 가 **바로 이 함정을 문서화하고**
`now_kst()` 로 막아뒀는데, 2026-08-19 에 들어온 이 경로만 가드 없이 지나갔다.

**해소(2026-08-31)**: `now_kst().date()` 로 통일했다(`management_block_reason` 의 신선도
판정도 같은 기준으로). 현재 beat 는 18:40 KST(=09:40 UTC)라 날짜가 우연히 일치해 증상이
없었지만, 확인 시점(21:10 UTC)에 `date.today()` 는 8/30, KST 는 8/31 이었다. 회귀 테스트
1건 신설.

## 66. 백테스트가 실행마다 다른 수치를 냈다 — set 순회 의존 ✅

`_apply_rebalance` 가 `set(targets) | set(val)` 을 그대로 순회해 주문을 만들었다. CPython
은 프로세스마다 `PYTHONHASHSEED` 로 문자열 해시를 무작위화하므로 이 순서가 실행마다
달라지고, 그 순서가 sells/buys 리스트 순서 → 체결 루프의 `cash` 순차 누적 순서가 되어
**같은 입력이 실행마다 다른 성과지표**를 냈다(합성 2년 백테스트 실측
alpha `0.14965995885546152` vs `...135`, trades 배열 순서도 변동).

크기는 1e-15 수준이라 전략 판정을 뒤집지는 않지만 두 가지를 망가뜨린다. (1) 백테스트
재현성 — 같은 전략을 두 번 돌리면 수치가 미세하게 다르다. (2) 이 함수에 수치 회귀
테스트를 걸 수 없다 — 기대값을 못 박으니 §67 같은 리팩토링의 전후 동일성도 검증
불가능했다.

**해소(2026-08-31)**: `sorted()` 한 단어. 회귀 가드는 서브프로세스 2개를 서로 다른
`PYTHONHASHSEED` 로 띄워 대조한다(`test_rebalance_determinism.py`) — 같은 프로세스
안에서는 시드가 고정이라 드러나지 않으므로 in-process 대조로는 잡히지 않는다(처음
그렇게 짰다가 정렬을 되돌려도 통과하는 것을 보고 바꿨다).

**같은 결함이 실거래 경로에도 있었다(영향은 더 크다)**: `engine/rebalance.py::
compute_rebalance_orders` 도 같은 식으로 순회했는데, 이 반환 리스트는 **실제 주문
제출 순서**다. 매수 자금이 빠듯하면 앞선 주문부터 체결되므로 같은 계획이 실행마다
다른 바스켓을 체결한다(주문 집합은 같고 순서만 갈린다). 함수 주석이 백테스트
`_apply_rebalance` 와의 parity 를 명시하므로 양쪽 모두 정렬해 맞췄다. 이쪽은 dict
삽입 순서만 바꿔도 드러나서 in-process 테스트로 고정했다. `rebalance_runner.py` 의
`symbols = set(...)` 2곳은 시세 조회 집합·len 에만 쓰여 순서가 결과에 닿지 않는다.

## 67. `run_rebalance_backtest` 준비 로직 분리 ✅

588줄·중첩깊이 8 로 코드베이스 최대 함수였다(다음이 213줄·깊이 3). 시뮬레이션 루프
앞에 150여 줄의 준비 코드가 깔려 있어 기본값 하나를 확인하려 해도 백테스트 전체를
돌려야 했다.

**해소(2026-08-31)**: 응집된 준비 로직 3덩어리만 순수 헬퍼로 뽑았다 — `_adv_frame`,
`_with_sector_map`(이 수치 함수 안의 유일한 네트워크 I/O 였다), `_parse_panic_overlay`
→ `PanicOverlayParams`(frozen). 588 → 538줄, 단위 테스트 17건 신설. 특히 `_pof` 가
`or` 대신 None 검사를 쓰는 이유(`scale_in_confirm=0` 같은 "0 으로 끄기"가 기본값으로
되살아나면 안 된다)를 이제 직접 고정한다.

**일별 루프·패닉 상태기계는 의도적으로 두었다.** 깊이 8 은 상태기계의 실제 형태이고,
분기 하나하나에 측정 근거가 주석으로 붙어 있다(상장폐지 terminal/interior gap 구분,
회수율 0.9 의 종목별 실측 근거 등). 평탄화는 리팩토링을 가장한 재작성이고, 이 함수의
수치는 id=23 승격 근거다.

**검증 방법**: 합성 2년·40종목 패널에 전 오버레이(레짐·패닉·리스크레이어·ADV캡·
상하한가·상장폐지)를 켠 백테스트의 결과 dict 전체(스칼라 18개 + equity_curve/markers/
holdings/trades/factor_ic 해시)를 리팩토링 전후로 대조해 **완전 동일**을 확인했다.
§66 을 먼저 고치지 않았으면 이 대조 자체가 불가능했다. 첫 골든 설정에는
`max_sector_pct` 가 없어 `_with_sector_map` 의 실제 분기가 한 번도 안 타는 구멍이
있었으므로, 섹터 한도를 켜고 업종 매핑을 결정론적으로 스텁한 변형(체결 483→528건)
으로 한 번 더 대조해 그 경로까지 동일함을 확인했다.

**측정만 하고 남긴 것 — 팩터 계산 핫패스**: cProfile 결과 백테스트 시간의 75% 가
`_targets_at`(리밸런싱일마다 호출) 안의 `_score_factor_frame`·`_compute_stock_scores`
에 있다. 종목별로 `hist[code].dropna()` 후 vol/mdd 를 파이썬 루프로 도는 구조라
패널 전체 벡터화로 큰 폭 단축이 가능해 보이나, `c.dropna().tail(N)`(마지막 N 개
**유효 관측**)과 패널 단위 `tail(N)`(마지막 N **행**)은 결측이 섞인 종목에서 값이
달라진다 — 즉 수치가 바뀐다. 시간복잡도는 우선순위가 가장 낮은 기준이라 손대지
않았다. 착수한다면 §66 의 골든 대조를 그대로 쓰되 "동일"이 아니라 "허용 오차"로
기준을 낮춰야 하고, 그 결정 자체가 별도 판단이다.

**같은 이유로 되돌린 것**: `_vol_slippage_map` 이 전체 이력에 `pct_change` 를 돌린 뒤
마지막 20개만 쓰는 것을 `tail(21)` 로 한정해 봤으나 (1) 프로파일 상위에 없어 측정된
이득이 없었고 (2) 부동소수 합산 순서가 달라져 결과가 1e-15 흔들렸다. 측정되지 않은
최적화에 재현성을 내주는 거래라 원복했다.
