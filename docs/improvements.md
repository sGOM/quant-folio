...
**추가 수정(2026-08-01) — 월별 조회 캐시 ✅**: 위 (a)~(c)는 *실패했을 때* 조용히
번지지 않게 하는 방어일 뿐, **차단을 부른 요청량 자체는 줄이지 않았다.** 과거 시점의
구성종목·시총은 **불변 데이터**인데도 프로세스 캐시가 재실행마다 사라져 같은 요청을
처음부터 반복한 것이 근본 원인이다. 성공 결과를 디스크에 캐시해(`_DISK_CACHE_DIR`,
기본 `backend/.cache/krx_pit`) **재실행 시 KRX 요청이 0회**가 되게 했다. 캐시 히트는
`_session()`보다 먼저 확인하므로 로그인 자체가 나가지 않는다. 과거일만 캐시하고(당일은
확정 전), 빈 결과는 캐시하지 않는다(메모리 캐시와 같은 규약 — 생존편향 재유입 방지).
아울러 §47 스크립트의 워밍업을 고정 상수에서 **cfg 최장 룩백 유도**로 바꿔 첫 실행의
월별 조회도 19→15회로 줄였다.

**남은 한계**: 차단 해제까지 기다려야 하며, 해제 시점은 알 수 없다(캐시는 **첫 1회**
조회를 없애주지 못한다 — 차단 상태에서는 여전히 아무것도 못 받는다). 캐시가 채워지기
전의 장기 구간 첫 실행은 그대로 위험 구간이므로, 세션 수명을 늘리는 개선은 여전히 유효한
후보다.
...
> 원인은 §44-1(아래) — 차단이 풀린 뒤 `scripts/validate_id23_crash_2026.py` 재실행 필요.
> **이 구간에서 얻은 수치는 전부 빈 패널 위의 값이므로 폐기했다.**
...
**설계**: PIT KOSPI200 only, 4-arm(**현행(레짐만) / 레짐+패닉ON / 오버레이 전무 / B&H**),
종료일 2026-07-30·07-31 **양쪽 병기**, up-beta/down-beta 분리, `regime_exit`·`panic_confirm`
이벤트 로그의 **정확한 발생 일자 추출**이 가설 판별의 직접 증거.

> **arm 구성 정정(2026-08-01)**: 최초 설계의 '패닉 off' arm 은 id=23 에 `panic_overlay`
> 가 없으므로 현행과 **동일한 config** 가 되어 백테스트만 낭비했다(경고만 찍고 그대로
> 실행되고 있었다). 위 전제 정정에 맞춰 반대로 **패닉을 켠 변형**을 arm 으로 넣어 P2 를
> 검증한다. 오버레이 파라미터는 `run_panic_overlay_comparison.py` 의 대표안 A 와 동일하게
> 고정했다 — 여기서 새 값을 고르면 그 자체가 미검증 자유도가 된다. B&H 도 다른 arm 과
> **같은 지표 집합**(alpha/Sharpe/MDD)으로 계산하도록 고쳤다. 기존에는 `total_return`
> 만 찍어 아래 판정 규약(alpha/Sharpe)으로는 비교조차 불가능했고, 폭락 구간 방어형
> 검증에서 MDD 가 빠져 있었다.
...
- **반증 조건**: (현행 − 오버레이전무) 차이의 50% 이상이 7-31 하루에서 발생하면 단일일
  의존이므로 **어느 결론도 채택 불가**로 보고한다. 단, 그 차이 자체가 **0.5%p 미만**이면
  '단일일 의존'이 아니라 **차이 없음(판정 무의미)** 으로 따로 보고한다 — 하한 없이
  비중을 계산하면 차이가 0 에 가까울 때 값이 발산해 무조건 '채택 불가'로 오분류된다.
