...
- **범위 키 소스의 커버 구간은 "요청한 범위"이지 "받아온 행의 범위"가 아니다.**
  ...
  병합하지 않는다(사이에 거래일이 있었는지 단정할 수 없다). **이 전제는 코드로
  검증되지 않는다** — `cached_range` 는 소스가 요청 창을 조용히 부분 응답해도 이를
  감지하지 못하고 요청 범위 전체를 커버로 기록한다(잔여 위험은
  `docs/superpowers/specs/2026-08-08-index-ohlcv-coverage-design.md` §11).
