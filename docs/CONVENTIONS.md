...
- **파일 크기 신호**: 한 파일이 800줄을 넘고 서로 다른 관심사(상태·검증·표현 등)가 섞이기 시작하면 분해를 검토한다. 단, **수치 정확성이 검증된 백테스트 코어(상태기계)는 이득 없이 쪼개지 않는다** — 회귀 위험이 가독성 이득보다 크다. 다만 그 안의 **설정 파싱·외부 I/O 같은 준비 로직은 분리 대상이다** — 동작이 없어 수치가 바뀔 수 없고, 분리해야 단위 테스트가 가능해진다(표본: §67 의 `_adv_frame`·`_with_sector_map`·`_parse_panic_overlay`).
- **같은 입력이면 같은 수치가 나와야 한다(재현성).** 이건 취향이 아니라 **검증 가능성의 전제**다 — 결과가 실행마다 흔들리면 그 코드에는 수치 회귀 테스트를 걸 수 없고, 리팩토링 전후 동일성도 증명할 수 없다. 구체적 함정 둘: ① **`set` 을 순회해 순서 있는 결과를 만들지 않는다** — 문자열 해시는 프로세스마다 무작위화(`PYTHONHASHSEED`)되어 체결 순서가 실행마다 달라졌다(§66, 백테스트 1e-15 + 실거래는 **주문 제출 순서가 갈려 다른 바스켓 체결**). ② **부동소수 누적 순서를 바꾸는 최적화는 재현성과 맞바꾸는 거래다** — 프로파일로 이득을 먼저 측정하고, 이득이 없으면 하지 않는다(§67 에서 되돌린 사례).
...
- **결측은 sentinel(0)이 아니라 `None` 으로 표현한다.** 0 은 실제 값과 구분이 안 된다 — 시총 0원 상장사, 종가 0원은 없지만 코드는 그걸 모른다. 이 저장소에서 같은 결함이 **세 번** 났다(§50 `price` → §62 `market_cap` → §64 `StockMetric` 3필드). 계약을 바꿀 때 소비자 세 곳을 같이 본다.
  - **정렬**: 결측은 맨 뒤로 — `key=lambda x: (x.f is None, ...)`. `None` 과 숫자를 비교하면 `TypeError` 다.
  - **필터**: 더 엄격한 기준에서는 결측을 **탈락**시킨다 — 결측은 기준 충족을 증명할 수 없다.
  - **표시**: 프론트는 `lib/format.ts` 널 허용 헬퍼(`fmtPct`/`fmtNum`/`fmtKRW`/`fmtAmt`)로 `"-"` 를 렌더한다.
  - 도달 불가한 결측 가드는 계약을 바꾸지 말고 **가드를 지우고 불변식을 테스트로 고정**한다(§64 스크리너).
- **KRX 거래일은 KST 로 계산한다 — `date.today()` 금지.** 컨테이너 TZ 는 UTC 이고, `celery_app.timezone="Asia/Seoul"` 은 **beat 스케줄 표시에만** 적용될 뿐 태스크 본문에는 영향이 없다. 거래일·기준일을 다루면 `app.services.market.now_kst().date()` 를 쓴다. 배치 시각이 KST 09:00 이후면 우연히 날짜가 맞아떨어져 **증상 없이 잠복하다가**, 스케줄을 새벽대로 옮기는 순간 하루씩 어긋난다(§65 에서 실제로 발생).
