...
> ⚠️ **문서 성격 안내(2026-07-16)**: 이 문서는 2026-06-21 작성된 **최초 기획(v0.1, MVP 범위)** 이다.
...
- **스택을 선택한 이유**:
  ...
  2. **Next.js (React) 프론트엔드**: SSR 기반으로 대시보드 초기 로딩이 빠르고, TypeScript 타입 안정성으로 실시간 모니터링 UI를 효율적으로 구축할 수 있다. 차트는 외부 라이브러리 의존을 피해 경량 자체 SVG 컴포넌트로 구현한다.
...
- **Styling**: Tailwind CSS, shadcn 스타일 UI(Radix 기반), 경량 자체 SVG 차트(`components/LineChart.tsx`)
- **Icons**: Lucide React
- **Database / Infra**: PostgreSQL + TimescaleDB, Redis, Docker Compose
- **증권사(브로커)**: 한국투자증권 KIS(기본, REST+WebSocket)·토스증권(REST 폴링) — `services/broker` 추상화로 주입
- **데이터 소스**: pykrx, FinanceDataReader, OpenDART(재무·팩터)

---

## 4. 데이터베이스 구조

- **users**: `id, email, password_hash, display_name, broker, kis_app_key(enc), kis_app_secret(enc), kis_account_no, toss_app_key(enc), toss_app_secret(enc), toss_account_no, created_at` — 사용자 계정. `broker`(kis/toss)로 주문 증권사 선택. 주문 자격증명은 `kis_*` 컬럼을 공통 재사용하고, `toss_*` 는 통합 시세 조회용(주문 브로커와 독립). 모든 키는 암호화 저장
