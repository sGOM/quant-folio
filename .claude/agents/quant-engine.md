...
- **같은 입력이면 같은 수치가 나와야 한다(재현성).** 결과가 실행마다 흔들리면 그 코드에는 수치 회귀 테스트를 걸 수 없고 리팩토링 안전성도 증명할 수 없다. 특히 **`set` 을 순회해 순서 있는 결과(주문 리스트·체결 순서)를 만들지 않는다** — 문자열 해시가 프로세스마다 달라진다. 누적 순서를 바꾸는 최적화는 프로파일로 이득을 먼저 측정한다. 상세는 `docs/CONVENTIONS.md` §0.
...
- **입력 조회 실패는 백테스트를 멈춘다 — 이건 의도된 동작이다.** 확정 과거 데이터는 로컬 우선으로 읽히고(`app/services/data/store/`), 미적재 구간에서 외부가 막히면 `DataSourceError` 로 중단된다. 조용히 빈 패널 위에서 완주하던 이전보다 낫다. 저하 규칙은 `docs/CONVENTIONS.md` §1 "외부 데이터 조회 계약" 참고 — 백테스트·리밸런싱 경로는 **전파**가 기본이고, 보조 지표만 항목 단위로 흡수한다.
- **실거래 엔진에서 판단 근거를 못 얻으면 무행동이 정답이다.** 레짐 기준지수 조회가 실패하면 "위험선호로 간주하고 주문"이 아니라 그 틱을 실패로 기록하고 다음 주기에 재시도한다(무행동=보유 유지이므로 신규 매수보다 항상 덜 공격적). 데이터 부재를 정상으로 뭉개면서 실제 자금을 움직이는 것이 §44-1/§47 의 최악 형태다.
