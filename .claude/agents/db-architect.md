---
name: db-architect
description: PostgreSQL + TimescaleDB 스키마 설계·마이그레이션·쿼리 최적화에 사용. 테이블 정의(users, strategies, backtests, strategy_likes, orders, executions, price_ticks, positions, risk_limits), TimescaleDB hypertable 구성, 인덱스, 마이그레이션 작성, 시계열 쿼리 튜닝 시 호출.
...
- 실제 스키마(`backend/app/models/models.py`)의 9개 테이블: users, strategies, backtests, strategy_likes, orders, executions, price_ticks, positions, risk_limits
